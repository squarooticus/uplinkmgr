...
def dhcpcd_conf(cfg: Config, uplink: UplinkConfig) -> str:
    lines = [
        f"# Generated by uplinkmgr-setup for uplink: {uplink.name}",
        HEADER.strip().split("\n")[1],  # "# Regenerate with: uplinkmgr-setup"
        "",
    ]
    ...
    if uplink.ipv6_pd:
        ...
        all_ifaces = [uplink.interface] + macvlan_ifaces
        lines.append(f"allowinterfaces {' '.join(all_ifaces)}")
        lines.append("")
        ...
        lines.append(f"allowinterfaces {uplink.interface}")
        lines.append("")
        lines.append(f"interface {uplink.interface}")
        lines.append(f"    metric {uplink.metric}")
...
def dhcpcd_unit(cfg: Config, uplink: UplinkConfig) -> str:
    pid_file = f"/run/dhcpcd/{uplink.interface}.pid"
    return (
        f"# Generated by uplinkmgr-setup for uplink: {uplink.name}\n"
        f"# Do not edit by hand.\n"
        f"\n"
        f"[Unit]\n"
        f"Description=DHCP client for uplinkmgr uplink: {uplink.name} ({uplink.interface})\n"
        f"After=network.target sys-subsystem-net-devices-{uplink.interface}.device\n"
        f"Wants=sys-subsystem-net-devices-{uplink.interface}.device\n"
        f"Before=uplinkmgr.service\n"
        f"\n"
        f"[Service]\n"
        f"Type=forking\n"
        f"PIDFile={pid_file}\n"
        f"ExecStart=/usr/sbin/dhcpcd --config {naming.dhcpcd_conf_path(uplink.name)} \\\n"
        f"    --nobackground {uplink.interface}\n"
        f"ExecStop=/usr/sbin/dhcpcd --exit --config {naming.dhcpcd_conf_path(uplink.name)} "
        f"{uplink.interface}\n"
